...
use crate::{
    helpers::{
        permissions,
        utils::{create_new_timer, deconstruct_time, get_voice_state, join_voice_internal},
    },
    structures::errors::{BardError, PermissionType},
    BotId, Lavalink, SpotifyClient, VoiceTimerMap,
};
...
async fn play(ctx: &Context, msg: &Message, args: Args) -> CommandResult {
    ...
    if manager.get(guild_id).is_none() {
        join_voice_internal(ctx, msg, voice_channel).await?;
    }
    ...
    let query = if args_message.contains("https://open.spotify.com") {
        let track_id = match args_message.rsplit('/').next() {
            ...
            None => {
                msg.channel_id
                    .say(ctx, BardError::Missing("valid Spotify URL"))
                    .await?;
                return Ok(());
            }
        };
...
async fn pause(ctx: &Context, msg: &Message) -> CommandResult {
    ...
    if lava_client.nodes().await.contains_key(&guild_id.0) {
        ...
        let ctx_clone = ctx.clone();
        tokio::spawn(async move {
            create_new_timer(ctx_clone, guild_id).await;
        });
    };
...
async fn clear(ctx: &Context, msg: &Message) -> CommandResult {
    ...
    if !permissions::check_permission(ctx, msg, None, false).await? {
        msg.channel_id
            .say(
                ctx,
...
async fn remove(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    let clear_num = match args.single::<usize>() {
        Ok(size) => {
            if size == 0 {
                msg.channel_id
                    .say(ctx, BardError::Missing("number greater than 0"))
                    .await?;
        ...
        Err(_) => {
            msg.channel_id
                .say(ctx, BardError::Missing("number"))
                .await?;
...
async fn seek(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    let time = match deconstruct_time(args.single::<String>().unwrap()) {
        Ok(time) => time,
        Err(e) => {
            msg.channel_id
                .say(ctx, BardError::Missing(&format!("valid amount of {}", e)))
                .await?;
